...
class PropositionExtractorService
{
    ...
    public function getPropositions(string $text): array
    {
...
' . $text;
    ...
    public function extractFromParagraphs(string $fullText): array
    {
        $paragraphs = array_filter(explode("\n\n", $fullText));
        $allPropositions = [];

        foreach ($paragraphs as $i => $paragraph) {
            $propositions = $this->getPropositions($paragraph);
            $allPropositions = array_merge($allPropositions, $propositions);
            echo "Done with paragraph " . ($i + 1) . "\n";
